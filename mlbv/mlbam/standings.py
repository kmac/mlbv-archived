...
STANDINGS_URL = (
    "https://statsapi.mlb.com/api/v1/standings/{standings_type}?"
...
def get_standings(standings_option="all", date_str=None, args_filter=None):
    ...
    if date_str == time.strftime("%Y-%m-%d"):
        ...
        date_str = None
    if util.substring_match(standings_option, "all") or util.substring_match(
        standings_option, "division"
    ):
    ...
    if util.substring_match(standings_option, "all") or util.substring_match(
        ...
        _display_standings(
            ...
            date_str,
            args_filter,
            rank_tag="wildCardRank",
            header_tags=("league",),
        )
        if util.substring_match(standings_option, "all"):
            print("")
    if (
        util.substring_match(standings_option, "all")
        or util.substring_match(standings_option, "overall")
        or util.substring_match(standings_option, "league")
        or util.substring_match(standings_option, "conference")
    ):
        _display_standings(
            "byLeague",
            "League",
    ...
    if util.substring_match(standings_option, "playoff") or util.substring_match(
        standings_option, "postseason"
    ):
...
def _get_standings_display_for_record(
    outl, standings_type, record, header_tags, rank_tag, border, needs_line_hr
):
    if standings_type == record["standingsType"]:
        ...
        LOG.error(
            "Unexpected: standingsType=%s, not %s",
...
def _display_standings(
    ...
    json_data = request.request_json(
        url,
        "standings-{}-{}".format(season_str, standings_type),
        cache_stale=request.CACHE_SHORT,
    )
    ...
    for record in json_data["records"]:
        if (
            args_filter
            and standings_type == "byDivision"
            and args_filter in mlbapidata.DIVISION_FILTERS
        ):
            pass

        _get_standings_display_for_record(
            outl, standings_type, record, header_tags, rank_tag, border, needs_line_hr
        )
...
def display_division_standings(
    date_str, args_filter, rank_tag="divisionRank", header_tags=("league", "division")
):
